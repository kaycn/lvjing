...
class CommonController extends Controller
{
      ...
      protected function _initialize()
      {
      	$id = I("get.id");
      	$fenlei = M("category");
        $code = $fenlei->where("fid = 0")->select();
        $this -> assign("code",$code);
            ...
            if(!(bool)array_key_exists($c,$_SESSION['code'])){
            	if(!($_SESSION['code']===0)){
            		$this -> error("你没有权限",U('Admin/Index/index'), 5);
            	}
            	
            }
    
            if(strtolower(CONTROLLER_NAME)=="demoa"){
            	if(!(bool)array_key_exists($id,$_SESSION['code'])){
            		if(!($_SESSION['code']===0)){
            		$this -> error("你没有权限",U('Admin/Index/index'), 5);
            		}
            	}
